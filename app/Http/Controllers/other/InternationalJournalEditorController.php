...
class InternationalJournalEditorController extends Controller
{
    ...
    public function download(Request $request) {
        ...
        $internationaljeditor         = $internationaljeditor->orderBy($sortBy, $orderBy)->get()->toArray();
        $internationaljeditor_array[] = array(/*'索引值',*/ '一級單位', '系所部門', '期刊編輯者', '期刊名稱', '開始擔任時間', '結束擔任時間', '備註');
        // dd($internationaljeditor);
        foreach ($internationaljeditor as $internationaljeditor_data) {
            $internationaljeditor_array[] = array(
                // 'id'          => $internationaljeditor_data['id'],
                'chtCollege'  => $internationaljeditor_data['chtCollege'],
                'chtDept'     => $internationaljeditor_data['chtDept'],
                'name'        => $internationaljeditor_data['name'],
                'journalName' => $internationaljeditor_data['journalName'],
                'startDate'   => $internationaljeditor_data['startDate'],
                'endDate'     => $internationaljeditor_data['endDate'],
                'comments'    => $internationaljeditor_data['comments'],
            );
        }

        Excel::create('擔任國際期刊編輯', function ($excel) use ($internationaljeditor_array) {
            $excel->setTitle('擔任國際期刊編輯');
            $excel->sheet('表單', function ($sheet) use ($internationaljeditor_array) {
                ...
                // dd($internationaljeditor_array);
                $sheet->fromArray($internationaljeditor_array, null, 'A1', false, false);
            });
        })->download('xlsx');
    }
