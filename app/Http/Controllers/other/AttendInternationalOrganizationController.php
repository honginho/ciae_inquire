...
class AttendInternationalOrganizationController extends Controller
{
    ...
    public function download(Request $request) {
        ...
        $attendiorganization         = $attendiorganization->orderBy($sortBy, $orderBy)->get()->toArray();
        $attendiorganization_array[] = array(/*'索引值',*/ '一級單位', '系所部門', '參加人', '組織名稱', '開始時間', '結束時間', '備註');
        // dd($attendiorganization);
        foreach ($attendiorganization as $attendiorganization_data) {
            $attendiorganization_array[] = array(
                // 'id'           => $attendiorganization_data['id'],
                'chtCollege'   => $attendiorganization_data['chtCollege'],
                'chtDept'      => $attendiorganization_data['chtDept'],
                'name'         => $attendiorganization_data['name'],
                'organization' => $attendiorganization_data['organization'],
                'startDate'    => $attendiorganization_data['startDate'],
                'endDate'      => $attendiorganization_data['endDate'],
                'comments'     => $attendiorganization_data['comments'],
            );
        }

        Excel::create('參與國際組織', function ($excel) use ($attendiorganization_array) {
            $excel->setTitle('參與國際組織');
            $excel->sheet('表單', function ($sheet) use ($attendiorganization_array) {
                ...
                // dd($attendiorganization_array);
                $sheet->fromArray($attendiorganization_array, null, 'A1', false, false);
            });
        })->download('xlsx');
    }
