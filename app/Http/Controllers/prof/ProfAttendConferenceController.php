...
class ProfAttendConferenceController extends Controller
{
    ...
    public function download(Request $request) {
        ...
        $Pattendconference         = $Pattendconference->orderBy($sortBy, $orderBy)->get()->toArray();
        $Pattendconference_array[] = array(/*'索引值',*/ '一級單位', '系所部門', '姓名', '身份', '前往國家', '會議名稱', '發表論文名稱', '開始時間', '結束時間', '備註');
        // dd($Pattendconference);
        foreach ($Pattendconference as $Pattendconference_data) {
            $profLevel = $Pattendconference_data['profLevel'];
            ...
            $Pattendconference_array[] = array(
                // 'id'                 => $Pattendconference_data['id'],
                'chtCollege'         => $Pattendconference_data['chtCollege'],
                'chtDept'            => $Pattendconference_data['chtDept'],
                'name'               => $Pattendconference_data['name'],
                'profLevel'          => $profLevel,
                'nation'             => $Pattendconference_data['nation'],
                'confName'           => $Pattendconference_data['confName'],
                'publishedPaperName' => $Pattendconference_data['publishedPaperName'],
                'startDate'          => $Pattendconference_data['startDate'],
                'endDate'            => $Pattendconference_data['endDate'],
                'comments'           => $Pattendconference_data['comments'],
            );
        }

        Excel::create('本校教師赴國外出席國際會議', function ($excel) use ($Pattendconference_array) {
            $excel->setTitle('本校教師赴國外出席國際會議');
            $excel->sheet('表單', function ($sheet) use ($Pattendconference_array) {
                ...
                // dd($Pattendconference_array);
                $sheet->fromArray($Pattendconference_array, null, 'A1', false, false);
            });
        })->download('xlsx');
    }
